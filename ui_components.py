...
def display_instructions():
    ...
    with st.expander("📖 How to Use This Tool", expanded=False):
        ...
        **Welcome to the Artwork Verification Dashboard! This tool helps you catch errors before they go to print.**
        ...
        1.  **📤 Upload Files**: Upload all related artwork files for a single product (`.pdf`, `.csv`, `.xlsx`).
        2.  **⚙️ Configure (Optional)**: In the sidebar, you can add required text (like "Made in China") for the tool to check for.
        3.  **🔍 Run Verification**: Click the "Run Verification" button to start the analysis.
...
def display_dashboard(global_results: List, per_doc_results: Dict, docs: List, skus: List):
    ...
    with col1:
        ...
        if total_failures > 0:
            ...
            st.success("**Overall Status: ✅ All Checks Passed**")
    
    with col2:
        if skus:
            st.info(f"**🔑 Key Information**: Detected SKU(s): `{', '.join(skus)}`")
        else:
            st.info("**🔑 Key Information**: No SKUs automatically detected.")
