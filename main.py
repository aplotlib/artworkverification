...
def main():
    ...
    api_keys = check_api_keys()
    run_validation, brand_selection, must_contain_text, must_not_contain_text, ai_provider, run_test_validation = display_sidebar(api_keys, current_batch_data)
    ...
    if run_validation and not uploaded_files:
        ...
        if run_test_validation:
            # ... (test file logic remains the same)
            pass
        else:
    ...
    if current_batch_data:
        ...
        display_dynamic_checklist(current_batch_data.get('brand', 'Vive'), st.session_state.current_batch_sku, current_batch_data)
