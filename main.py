...
import time
import psutil # Added as a core dependency

# --- Tesseract Configuration (Uncomment if needed for deployment) ---
# On some systems, especially cloud deployments, you may need to specify the path.
# Example for Linux:
    ...
    try:
            ...
            if uploaded_files:
                with st.spinner("Processing files..."):
                    ...
                    for idx, file in enumerate(uploaded_files):
                        try:
                            progress_bar.progress((idx + 1) / len(uploaded_files), f"Processing {file.name}...")
        ...
        if st.session_state.validation_complete:
            ...
            try:
                ...
                for doc_type, results in sorted(results_by_doc.items()):
                    if doc_type in st.session_state.documents:
                ...
                if report:
                    ...
                    if issues:
                        ...
                        cols = st.columns(4)
                        cols[0].metric("Issues Reviewed", f"{summary['reviewed']}/{summary['total_issues']}")
                        cols[1].metric("Fixed", summary['fixed'])
                        cols[2].metric("Will Fix Later", summary['will_fix_later'])
                        cols[3].metric("Disputed", summary['disputed'])
        ...
        if st.session_state.documents:
            ...
            if doc_keys:
                doc_type = st.selectbox("Select document to preview", doc_keys)
                
                if doc_type and doc_type in st.session_state.documents:
